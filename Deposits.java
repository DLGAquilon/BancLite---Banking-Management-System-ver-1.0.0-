    ...
    private JPanel depositPanel(String loginUser, String loginPass) {
        ...
        btnDeposit.addActionListener(e -> {
            ...
            try {
                ...
                if (!CredentialListing.validate(pass.trim())) {
                    JOptionPane.showMessageDialog(depositFrame, "Invalid Password!", "Error!", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                while ((currentLine = br.readLine()) != null) {
                    ...
                    if (lineData[7].trim().equals(pass.trim())) {
                        if (lineData.length <= 8) {
                            String[] expandedData = new String[9];
                        ...
                        // Parse current balance from index 8
                        double currentBalance = 0.00;
                        try {
                            currentBalance = Double.parseDouble(lineData[8].trim());
                        } catch (NumberFormatException | NullPointerException ex) {
                            currentBalance = 0.00;
                        }
                        double newBalance = currentBalance + depAmnt;
                        lineData[8] = String.valueOf(newBalance);
